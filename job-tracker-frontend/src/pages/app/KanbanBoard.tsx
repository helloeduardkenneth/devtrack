    ...
    return (
        <div
            ...
            className={`group cursor-grab rounded-xl border border-slate-200 bg-white p-4 transition-all hover:shadow-lg active:cursor-grabbing ${
                isDragging
                    ? 'scale-105 rotate-2 opacity-50 shadow-2xl'
                    : 'opacity-100'
            }`}
        >
            ...
            {job.priority && (
                <div className="mb-3 flex items-center gap-2">
                    <div
                        className={`h-2 w-2 rounded-full ${priorityColors[job.priority]}`}
                    />
...
const Column: React.FC<ColumnProps> = ({ title, jobs, moveJob, onAddCard }) => {
    ...
    return (
        <div className="w-80 shrink-0">
            {/* Column Header */}
            <div
                className={`${colors.bg} ${colors.border} mb-4 rounded-xl border p-4`}
            >
