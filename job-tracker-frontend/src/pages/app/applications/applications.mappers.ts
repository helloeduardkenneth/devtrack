...
export const mapJobTypeToForm = (
  ...
  const map: Record<IApplicationItem['job_type'], AddJobFormValues['jobType']> = {
    FULL_TIME: 'Full-time',
    CONTRACT: 'Contract',
    PART_TIME: 'Part-time',
...
export const mapPriorityToForm = (
  ...
  const map: Record<IApplicationItem['priority'], AddJobFormValues['priority']> = {
    HIGH: 'High',
    MEDIUM: 'Medium',
    LOW: 'Low',
  }
...
export const mapStatusToUi = (
  ...
  const statusMap: Record<IApplicationItem['status'], ApplicationRecord['status']> = {
    APPLIED: 'Applied',
    PHONE_SCREEN: 'Phone Screen',
    TECHNICAL: 'Technical',
    ONSITE: 'Onsite',
...
export const formatSalaryRange = (salaryMin: number | null, salaryMax: number | null) => {
...
export const mapApplicationToRecord = (item: IApplicationItem): ApplicationRecord => ({
  id: String(item.id),
